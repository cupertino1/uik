...
Функция ВидПериодаОтчета(Форма, ОргДатаРегистрации)
	...
	Если (ДатаРегистрации > ДатаНачалаОбычногоПериода И ДатаРегистрации < Форма.мДатаКонцаПериодаОтчета) Тогда
		ВидПериода = "Сокращенный";
		
	ИначеЕсли (ДатаРегистрации >= ДатаРасширенногоПериода И ДатаРегистрации < ДатаНачалаОбычногоПериода) Тогда
		ВидПериода = "Расширенный";
		
	Иначе
		ВидПериода = "Стандартный";
...
Процедура ОткорректироватьНачальныйПериод(ВыбОрганизация)
	...
	РасширениеПериода = (ДатаРегистрации >= Дата(Год(ДатаРегистрации), 10, 1));
	
	Если мДатаКонцаПериодаОтчета < ДатаРегистрации Тогда
		Если РасширениеПериода Тогда
			Если ПредставляетсяЗаГод Тогда
...
Процедура ОткрытьФормуОтчета(Команда)
	...
	Форма = ОткрытьФорму(СтрЗаменить(ЭтаФорма.ИмяФормы, "ОсновнаяФорма", "") + мВыбраннаяФорма, ПараметрыФормы);
...
Процедура УстановитьДоступностьФлажкаБалансаНКО(Форма)
	...
	Иначе
		...
		Форма.Элементы.НадписьОрганизацияНекоммерческая.Видимость = Ложь;
...
Процедура ВосстановитьНастройкиНКО(ВыбОрганизация)
	...
	Если НЕ (Элементы.ПереключательНКО.Доступность и Элементы.ПереключательНКО.Видимость) Тогда
		...
		// Отчет скопирован. 
		// Или создан за период, в котором отдельного баланса НКО еще не было.
		Возврат;
	
	КонецЕсли; 

	НастройкиНКОизНастроек = ХранилищеНастроекДанныхФорм.Загрузить("Отчет.РегламентированныйОтчетБухОтчетность.Форма.ОсновнаяФорма", 
														"НастройкиНКО");
	Если НастройкиНКОизНастроек = Неопределено Тогда
	    // Настройки не найдены, ничего не делаем
	Иначе
...
Процедура СохранитьНастройкиНКО(ВыбОрганизация)
	...
	ХранилищеНастроекДанныхФорм.Сохранить("Отчет.РегламентированныйОтчетБухОтчетность.Форма.ОсновнаяФорма", 
...
Процедура КонтрагентПриИзмененииНаСервере()
    ДоговорДУ = Справочники.ДоговорыКонтрагентов.Выбрать(,Контрагент);
	Если ДоговорДУ.Следующий() Тогда
 		ДоговорКонтрагента = ДоговорДУ.Ссылка;
	КонецЕсли
КонецПроцедуры
