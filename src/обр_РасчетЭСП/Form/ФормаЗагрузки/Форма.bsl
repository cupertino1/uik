...
//--------------------------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------------------------
Функция ЦелеваяФункция(ПараметрыЦелевойФункции)
	Основание = 1 + ПараметрыЦелевойФункции.Предположение; 
	Результат = 0;
	Для Сч = 0 По ПараметрыЦелевойФункции.ДенежныйПоток.ВГраница() Цикл 
		Степень = ((ПараметрыЦелевойФункции.ПотокДат[Сч] - ПараметрыЦелевойФункции.ПотокДат[0])/86400)/365;
		Результат = Результат + ПараметрыЦелевойФункции.ДенежныйПоток[Сч] / Pow(Основание, Степень);
    КонецЦикла;

	Возврат Результат;	
КонецФункции
//--------------------------------------------------------------------------------------------------------------
Функция НайтиКореньМетодомЗолотогоСечения(ИмяЦелевойФункции, ПараметрыЦелевойФункции) Экспорт
	ЗначениеЦелевойФункции = 0;
	ЗолотойКоэффициент = 2 - (sqrt(5) + 1) / 2;           
		
	Выполнить("ЗначениеЦелевойФункции = " + ИмяЦелевойФункции + "(ПараметрыЦелевойФункции);");
	Шаг = ?(ЗначениеЦелевойФункции > 0, 1, -1) * (ЗолотойКоэффициент*ПараметрыЦелевойФункции.Предположение);
    Пока МодульЧисла(Шаг) > ПараметрыЦелевойФункции.Точность Цикл
		ПараметрыЦелевойФункции.Предположение = ПараметрыЦелевойФункции.Предположение + Шаг;
		Выполнить("ЗначениеЦелевойФункции = " + ИмяЦелевойФункции + "(ПараметрыЦелевойФункции);");
        Шаг = ?(ЗначениеЦелевойФункции*Шаг < 0, -Шаг*ЗолотойКоэффициент, Шаг);
    КонецЦикла; 
            
	Возврат ПараметрыЦелевойФункции.Предположение;
КонецФункции
...
Процедура ПрочитатьФайлНаСервере(Парам)
    ...
    Иначе
        ЭСП = орт_ФинансовыйКалькулятор.РасчетЭСП(ДенежныйПоток, ПотокДат, Объект.НачальноеПредположение, 0.000000001, Парам);
    КонецЕсли;
КонецПроцедуры
//--------------------------------------------------------------------------------------------------------------
&НаКлиенте
